...
if [[ "$debug" == 1 ]];then
    args=(
        go build 
        -ldflags "\"-s -w -X 'github.com/zuiwuchang/xray_webui/version.Version=$Version' -X 'github.com/zuiwuchang/xray_webui/version.Date=$date' -X 'github.com/zuiwuchang/xray_webui/version.Commit=$commit'\""
        -o "bin/$target"
    )
else
    args=(
